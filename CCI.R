...
autoplot(CCIw)
autoplot(diff(CCIw))
timeseries = ts(cbind(WEI_data$WEI,as.vector(CCIw)), start = 2008 ,frequency=365.25/7)
timeseries = ts(cbind(as.vector(diff(WEI)),as.vector(diff(CCIw))), start = 2008 ,frequency=365.25/7)
autoplot(timeseries)

cor(as.vector(CCIw),as.vector(WEI))
